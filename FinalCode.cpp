	...
	for (auto i : itemsets) if (i.F.size() == max_items) {
		  #pragma omp for
			for (int j = 1; j < (1 << max_items) - 1; j++) {
				...
				bool ok = false;
				for (auto i : tset) if (i >= 13) ok = true;
				if (ok && centage >= min_conf) assoc_rules.push_back (make_pair (sset, tset));
			}
	}
}

/*
Environment Variables : - Temperature (1 - 3), Wind Speed (4 - 6), Rainfall (7 - 9), Moisture (10 - 12)
Fire (13 - 15) : - Weak Fire, Normal, Fast Spreading
*/
